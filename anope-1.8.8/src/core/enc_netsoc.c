...
static int netsoc_encrypt(const char *src, int len, char *dest, int size)
{
    SHA1_CTX context;
    char tmp[41];
...
static int netsoc_encrypt_check_len(int passlen, int bufsize)
{
    if (bufsize < 23)
...
static int netsoc_decrypt(const char *src, char *dest, int size)
{
    return 0;
}
...
static int netsoc_check_password(const char *plaintext, const char *password)
{

    /* Unix crypt password hash */
    if(password[20] == 'X' && password[21] == 'X' && password[22] == 'X')
    {
        char* encr = crypt(plaintext, password);
        int cmpresult = strcmp(encr, password);
    ...
    if (netsoc_encrypt(plaintext, strlen(plaintext), buf, sizeof(buf)) < 0)
        return -1;
...
int AnopeInit(int argc, char **argv) {
    ...
    encmodule_encrypt(netsoc_encrypt);
    encmodule_encrypt_check_len(netsoc_encrypt_check_len);
    encmodule_decrypt(netsoc_decrypt);
    encmodule_check_password(netsoc_check_password);
